...
|----------|----------------|
| Database primary key (internal) | UUID v4 or auto-increment |
| Public API resource ID | 32-byte random |
| Session token | 32-byte random |
| Password reset token | 32-byte random |
| File access URL | 32-byte random |
| Shareable secret link | 32-byte random |
